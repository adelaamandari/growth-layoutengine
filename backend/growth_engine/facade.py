...
# Returned by _choose for a wall meant to stay open. Not a panel key,
# and deliberately not None -- the caller has to notice it.
OPEN = "-"
...
def _choose(kind: str, level: int, run_rank: int, n: int, i: int) -> tuple[str, str]:
    ...
    The rules are Adela's, one per line:
      ...
      corridor         --  open deck access: left unclad entirely. Only
                            reaches here when the corridor is NOT between
                            two-sided building, which needs no test: an
                            exterior wall has one owner, so a
...
def build_facade(plan: FloorPlan, pitch_cm: float | None = None,
    ...
    for ln in _lines(exterior):
        ...
        for level in sorted(by_level):
            ...
            #
            # The pass above fires only when a whole storey-line placed
            # nothing. A line usually carries several walls, so a wall can
            # come out bare while its neighbours on the same line are
            # clad and the fallback never triggers -- the bays simply
            # landed next door. That left 8.2m of unit wall bare on the
            # default plan with no reason a reader could see.
            #
            # Adela's rule is that a residential unit gets cladding on
