...
- **Open decks have no guarding drawn.** There is no balustrade among the nine
  panels, so a deck-access walkway three storeys up currently reads as an open
  edge. A synthesised one was built twice and removed twice — it is not a
  geometry problem, it is that the part does not exist yet;
  `git log --grep "Panel J"` finds the working version.
